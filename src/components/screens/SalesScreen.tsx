...
<table><thead><tr><th>SN</th><th>TYPE</th><th>CARTON/PIECE</th><th>CATEGORY</th><th>PRODUCT NAME</th><th class="r">SQFT/QTY</th><th class="r">PRICE</th><th class="r">SUB TOTAL</th></tr></thead><tbody>${sale.items.map((item, idx) => {
  ...
  return '<tr><td>' + (idx+1) + '</td><td>' + (item.itemType || 'Sale') + '</td><td>' + cartonPiece + '</td><td>' + (item.category || p?.category || '') + '</td><td class="b">' + item.name + (p?.size ? ' (Size: ' + p.size + ')' : '') + '</td><td class="r">' + Number(item.sqftQty ?? 0).toFixed(2) + '</td><td class="r">' + item.price + '</td><td class="r b">' + Math.round((item.sqftQty ?? item.qty) * item.price) + '</td></tr>';
}).join('')}</tbody></table>
