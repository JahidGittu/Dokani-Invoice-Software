...
<div class="disclaimer">বিক্রিত মাল ১ মাসের মধ্যে ফেরত নেওয়া হয়।</div>
  ...
  const generatePDF = async (sale: SaleRecord) => {
    ...
    const pw = 210; let y = 20;
    doc.setFillColor(0, 92, 193); doc.roundedRect(15, y - 4, 12, 12, 2, 2, 'F');
    ...
    try { const qrUrl = await QRCode.toDataURL(`${sale.invoice}-${sale.total}`, { width: 80, margin: 1 }); doc.addImage(qrUrl, 'PNG', pw - 33, y - 4, 16, 16); } catch {}
    doc.setFontSize(12); doc.setTextColor(0, 92, 193); doc.setFont('helvetica', 'bold'); doc.text(sale.invoice, pw - 15, y + 22, { align: 'right' });
    y += 16; doc.setDrawColor(0, 92, 193); doc.setLineWidth(0.8); doc.line(15, y, pw - 15, y); y += 8;
    doc.setFillColor(245, 247, 248); doc.roundedRect(15, y - 3, pw - 30, 14, 2, 2, 'F');
    doc.setFontSize(9); doc.setTextColor(45, 52, 53); doc.setFont('helvetica', 'bold'); doc.text(sale.customer, 18, y + 6);
    y += 16;
    const tableData = sale.items.map(item => [item.name, String(item.qty), formatCurrency(item.price), formatCurrency(item.price * item.qty)]);
    doc.autoTable({ startY: y, head: [['Product', 'Qty', 'Rate', 'Total']], body: tableData, theme: 'striped', margin: { left: 15, right: 15 }, styles: { fontSize: 9, cellPadding: 3 }, headStyles: { fontStyle: 'bold', fontSize: 8, textColor: [90, 96, 97], fillColor: [245, 247, 248] } });
    y = doc.lastAutoTable.finalY + 8;
    const totalsX = 140;
    doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text('TOTAL', totalsX, y); doc.setTextColor(0, 92, 193); doc.text(formatCurrency(sale.total), pw - 15, y, { align: 'right' }); y += 6;
    doc.setFontSize(9); doc.setTextColor(0, 97, 32); doc.text('Paid', totalsX, y); doc.text(formatCurrency(sale.paid ?? sale.total), pw - 15, y, { align: 'right' }); y += 5;
    if ((sale.due ?? 0) > 0) { doc.setTextColor(159, 64, 61); doc.text('Due', totalsX, y); doc.text(formatCurrency(sale.due!), pw - 15, y, { align: 'right' }); }
